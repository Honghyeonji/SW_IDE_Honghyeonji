...
// Distance sensor
#define _DIST_ALPHA 0.5  // [3162] ema 필터의 alpha 값을 0.0으로 설정
#define LENGTH 30
#define k_LENGTH 8
...
float dist_raw, dist_ema, dist_celi;    //[3160] 실제 거리측정값과 ema필터를 적용한 값을 저장할 변수
...
// convert angle speed into duty change per interval.
  duty_chg_per_interval = (float)(_DUTY_MAX - _DUTY_MIN) * _SERVO_SPEED / _SERVO_ANGLE * _INTERVAL_SERVO / 1000;     //[3159] 서보 각속도에 맞추어 센서 인식 간격 변경? [3150] 서보의 각속도를 원하는 Angle로 나누어 interval을 설정한다.
}
...
////////////////////
  ...
  // get a distance reading from the distance sensor
    dist_raw = ir_distance_filtered();  // [3157] 적외선 선세로 측정한 값에 필터를 적용한 값
//    if(dist_raw <= 129){ dist_celi = 100 + 300.0 / (129 - 75) * (dist_raw - 75); }
//    else if(dist_raw <= 183){ dist_celi = 100 + 300.0 / (183 - 129) * (dist_raw - 129); }
//    else if(dist_raw <= 230){ dist_celi = 100 + 300.0 / (230 - 183) * (dist_raw - 183); }
//    else if(dist_raw <= 286){ dist_celi = 100 + 300.0 / (286 - 230) * (dist_raw - 230); }
//    else if(dist_raw <= 323){ dist_celi = 100 + 300.0 / (323 - 286) * (dist_raw - 286); }
//    else if(dist_raw <= 357){ dist_celi = 100 + 300.0 / (357 - 323) * (dist_raw - 323); }

  // PID control logic
    error_curr = dist_target - dist_ema; // [3158] 목표값 에서 현재값을 뺀 값이 오차값
    pterm = _KP*error_curr; 
//   iterm += _KI*(error_curr - error_prev)
